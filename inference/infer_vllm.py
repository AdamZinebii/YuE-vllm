"""
YuE Music Generation Inference using vLLM

This script provides vLLM-based inference for the YuE music generation model.
vLLM offers faster inference compared to the HuggingFace transformers approach.

Note: vLLM does not support guidance_scale (Classifier-Free Guidance), 
so this parameter is not used in this implementation.
"""
...
print("Stage 1 inference using vLLM...")
for i, p in enumerate(tqdm(prompt_texts[:run_n_segments], desc="Stage1 inference...")):
    ...
    # Combine with previous output if not first segment
    if i > 1 and raw_output is not None:
        input_ids = raw_output.tolist() + prompt_ids
    else:
        input_ids = prompt_ids
    
    # Use window slicing if output sequence exceeds context
    ...
    # Ensure output ends with eoa token
    if generated_ids[-1] != mmtokenizer.eoa:
        generated_ids.append(mmtokenizer.eoa)
    
    # Combine input and output
    output_seq = input_ids + generated_ids
    
    if i > 1:
        raw_output = np.array(raw_output.tolist() + prompt_ids + generated_ids)
    else:
        raw_output = np.array(output_seq)
...
# Offload Stage 1 model
if not args.disable_offload_model:
    del model
    torch.cuda.empty_cache()

print("Stage 2 inference using vLLM...")
model_stage2 = LLM(
    model=stage2_model,
    dtype="bfloat16",
    trust_remote_code=True,
...
def stage2_generate(model, prompt, batch_size=16):
    ...
    # Teacher forcing generate loop for Stage 2
    # Note: vLLM doesn't directly support teacher forcing like HF transformers
    # We need to generate 7 tokens at a time for each frame
    all_outputs = []
    
    for batch_idx in range(len(prompt_ids_list)):
        current_prompt = prompt_ids_list[batch_idx].tolist()
        current_codec = codec_ids[batch_idx] if batch_size > 1 else codec_ids.flatten()
        
        # Generate 7 tokens for each codec frame
        for frames_idx in range(len(current_codec)):
            cb0 = int(current_codec[frames_idx])
            current_prompt.append(cb0)
            ...
            generated_ids = list(outputs[0].outputs[0].token_ids)
            current_prompt.extend(generated_ids)
        
        # Extract the generated tokens (after the initial prompt)
        len_initial_prompt = len(prompt_ids_list[batch_idx])
        output_tokens = current_prompt[len_initial_prompt:]
        all_outputs.append(np.array(output_tokens))
